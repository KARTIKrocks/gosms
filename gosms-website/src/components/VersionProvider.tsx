...
export default function VersionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch('https://api.github.com/repos/KARTIKrocks/gosms/releases', {
  ...
  function getReleaseUrl(version: string): string {
    ...
    return release?.html_url ?? `https://github.com/KARTIKrocks/gosms/releases/tag/${version}`;
  }

  function getInstallCmd(version: string): string {
    if (version === latestVersion) {
      return 'go get github.com/KARTIKrocks/gosms';
    }
    return `go get github.com/KARTIKrocks/gosms@${version}`;
  }
